...
public:
	...
	void storeRecord(int recNum) {
		auto os = p_simulation->getObjects();
		string root = "out/out.";
		size_t finddot = root.find_last_of(".");
		root = root.substr(0, finddot);
		Eigen::MatrixXd V;
		Eigen::MatrixXi F;
		int objectsSize = os.size();
		cout << "number of objects: " << objectsSize << std::endl;
		for (size_t i = 0; i < os.size(); i++) {
			os[i].getMesh(V, F);
			//taken from the exportrecord function
			string fileName = root + getName(objectsSize, i, recNum);
			if(!igl::writeOBJ(fileName, V, F)){
				cerr << "a problem in writing this object" << endl;
			}
			else{
				cout << "storing..." << getName(objectsSize, i, recNum) << endl; 
			}
...
protected:
	inline std::string getName(int objectsSize, int objectIndex, int recNum){
		std::stringstream name;
		name << "_object" << std::setw(std::max(2, static_cast<int>(std::log10(objectsSize)))) << std::setfill('0')
		<< objectIndex << "_" << std::setw(std::log10(m_maxSteps)) << recNum << ".obj";
		return name.str();
	}
